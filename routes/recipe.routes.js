...
router.post('/recipes', (req, res, next) => {
    ...
    if (!title || !cuisine || !dishType  || !level || !servings || !ingredients || !instructions) {
  ...
  router.get('/recipes/:recipeId', (req, res, next) => {
    ...
    .populate("author", "name email")    // Populate author details
      ...
      .catch((err) => {
        ...
        res.status(500).json({ message: "Error while getting a specific recipe" });
      });
  });
  ...
  router.put('/recipes/:recipeId', (req, res, next) => {
    ...
    Recipe.findByIdAndUpdate(
      recipeId, 
      { title, cuisine, dishType, level, duration, ingredients, instructions, image, author}, 
